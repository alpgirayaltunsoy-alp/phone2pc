...
in the background — the server auto-starts, and the GUI window is just a
viewer/configuration panel you can close without stopping anything.
...
- Every endpoint beyond basic status/stats requires a paired-device
  ...
  the one-time code.
- Remote file browsing and app launching are hard-restricted to the
  directories/applications you explicitly allow-list in Settings — nothing
  outside those lists is reachable from a phone, even with a valid token.
