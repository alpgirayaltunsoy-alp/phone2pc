    ...
    def start(self) -> None:
        with self._lock:
            ...
            uv_config = uvicorn.Config(
                ...
                log_level="warning",
                loop="asyncio",
                # We already have our own logging (core/logging_setup.py) feeding
                # the Logs tab, and uvicorn's own logging setup assumes a real
                # console stream (it calls stdout.isatty()), which crashes in a
                # windowed/no-console PyInstaller build where stdout is None.
                # log_config=None skips uvicorn's dictConfig entirely.
                log_config=None,
                use_colors=False,
            )
