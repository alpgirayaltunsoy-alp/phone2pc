...
class SettingsTab(QWidget):
    def __init__(self):
        ...
        self.remote_control_cb = QCheckBox("Enable remote screen & input control from paired phones")
        self.remote_control_cb.setChecked(ctx.config.remote_control_enabled)
        self.remote_control_cb.toggled.connect(
            lambda checked: ctx.config.set("remote_control_enabled", checked)
        )
        remote_note = QLabel(
            "Off by default. When on, any paired phone can view your screen and\n"
            "send keyboard/mouse input. Microphone and camera access are never\n"
            "exposed by this app."
        )
        remote_note.setStyleSheet("color: gray;")
        remote_layout.addWidget(self.remote_control_cb)
